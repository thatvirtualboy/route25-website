...
function isFuturePublicationSlot(date, now = new Date()) {
  const publishDate = date instanceof Date ? date : new Date(date);
  return Number.isFinite(publishDate.getTime()) && publishDate > now && isExactPublicationSlot(publishDate);
}
...
function publicationPreflight(issue, config = {}) {
  const isTest = issue?.isTest === true;
  const checks = [
    { key: "title", label: "Title", ok: Boolean(String(issue?.title || "").trim()), blocking: true },
    { key: "issue-number", label: "Issue number", ok: Number.isInteger(Number(issue?.issueNumber)) && Number(issue.issueNumber) > 0, blocking: true },
    ...
    isTest
      ? { key: "test-hidden", label: "Test issue safely hidden", ok: true, blocking: false }
      : { key: "not-test", label: "Ready for public publication", ok: true, blocking: true },
  ...
  if (issue?.status === "scheduled" || scheduledDate) checks.splice(7, 0, { key: "slot", label: "Saturday 7:00 AM Mountain slot", ok: Boolean(scheduledDate && isExactPublicationSlot(scheduledDate)), blocking: true });
  return {
    checks,
    readyToSchedule: !isTest && checks.filter(check => check.blocking).every(check => check.ok),
    readyToPublish: !isTest && checks.every(check => check.ok)
  };
}

module.exports = { ZONE, isWeeklyPublicationSlot, isExactPublicationSlot, isFuturePublicationSlot, isCurrentPublicationWindow, publicationPreflight };
