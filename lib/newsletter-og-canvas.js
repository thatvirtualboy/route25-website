...
function renderNewsletterSocialCanvas(issue, assets = {}) {
  const title = cleanText(issue?.title, "A Route 25 collector spotlight");
  const trainer = cleanText(issue?.trainerName, "Featured collector");
  const issueNumber = Number(issue?.issueNumber) > 0 ? `ISSUE #${String(Number(issue.issueNumber)).padStart(3, "0")}` : "COLLECTOR SPOTLIGHT";
