...
function buildEmail(issue) {
  ...
  const productGroup = (title, items, linkLabel) => items.length ? `<tr><td style="padding:18px 20px 8px;color:#168fc8;font-size:12px;font-weight:bold;letter-spacing:1.2px;text-transform:uppercase">${title}</td></tr>${items.map(product => `<tr><td style="padding:0 20px 12px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;background:#ffffff;border:1px solid #dce4ec;border-radius:12px"><tr><td style="padding:14px 16px;color:#475467;font-size:14px;line-height:1.45"><strong style="display:block;color:#101828;font-size:16px">${html(product.name)}</strong>${product.note ? html(product.note) : ""}</td>${product.url ? `<td align="right" style="padding:14px 16px;white-space:nowrap"><a href="${html(product.url)}" style="display:inline-block;color:#087bb5;font-size:13px;font-weight:bold;text-decoration:none">${linkLabel} →</a></td>` : ""}</tr></table></td></tr>`).join("")}` : "";
