...
test("complete issues can be scheduled with one, two, or three email images", () => {
  for (const count of [1, 2, 3]) {
    const issue = { issueNumber: 1, title: "Story", trainerName: "Trainer", summary: "Summary", images: Array.from({ length:count }, (_, index) => ({ objectPath:`newsletter/${index}.jpg` })), interviewAnswers: [{ answer: "A" }], lastTestEmailAt: new Date().toISOString(), isTest: false };
