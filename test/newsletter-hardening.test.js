...
test("collector spotlight presentation uses fixed subscription and structured editorial fields", () => {
  ...
  assert.match(adminPage, /Cards use the Route 25 card URL for “View card.”/);
  assert.match(previewPage, /'View card','route25Url',false/);
  assert.match(previewPage, /'Shop gear','url',true/);
  assert.match(publicRenderer, /"View card", "route25Url", false/);
  assert.match(publicRenderer, /"Shop gear", "url", true/);
  assert.match(newsletterApi, /"View card", "route25Url", false/);
  assert.match(newsletterApi, /"Shop gear", "url", true/);
  ...
  assert.match(publicRenderer, /noopener noreferrer sponsored nofollow/);
